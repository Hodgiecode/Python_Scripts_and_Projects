    ...
    for x in flist:
        if "PDF" or "pdf" in x:
            if predmet in x and not "МАГИСТР" in x:
                ...
                f.close()
                array,stats = sort(array, grant, contract)
...
def get_general(a_m,a_r,a_b,a_rs,a_l,profile,grant,contract):
    ...
    if profile=="ПМиИ": pivot_index = 2
    if profile=="Биология": pivot_index = 3
    if profile=="РЕКЛАМА И СВЯЗИ С ОБЩЕСТВЕННОСТЬЮ": pivot_index = 4
    if profile=="ФИЛОЛОГИЯ": pivot_index = 5
    ...
    array,stats = sort(data_list,grant,contract)
    ...
    f.write("№ "+"%-37s"%"ФИО"+"%-20s"%"Ф-т.\tРус.\tПрофил.\tОбщий балл\n")
    ...
    for i in range(len(array)):
        ...
        s = str(i+1)+"."+str(array[i][0][0])+"\t"+mode+"\t"+str(array[i][0][1])+"\t"+str(array[i][0][pivot_index])+"\t"+str(array[i][1])+"\n"
        f.write(s)
...
def main():
    url='https://msu.dvaoblaka.ru/api/tags/%D0%9D%D0%BE%D0%B2%D0%BE%D1%81%D1%82%D0%B8/articles?page=13'
